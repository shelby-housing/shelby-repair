...
case_active <- case_levels[3:9]

# Active + Completed
case_active_done <- case_levels[3:10]

# Project statuses with active contractors
contractor_active <- case_active[4:7]
...
raw_cases <- read_excel(fp, sheet = 2) |>
  janitor::clean_names() |>
  select(-c(1:3, year_built, clearance_date, starts_with(c("idis", "lien")))) %>% 
  mutate(
...
parid_cases <- parcels %>% 
  ...
  mutate(marker_label = factor(marker_label, levels = c(case_icons$marker_label)),
         label = paste0(
         ...
         label = case_when(
           ...
           project_status %in% c("Out For Bid", "Rebid") ~ 
